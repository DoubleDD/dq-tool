# dq-desktop — dq-tool 桌面版

dq-tool 的纯 JVM 桌面重写版:Kotlin + JetBrains Compose Multiplatform Desktop,去掉 Web 前端与 Spring Boot,UI 进程内直接调用服务层。
...
```bash
./gradlew run          # 启动桌面应用,数据目录默认 ./data
./gradlew test         # 全部测试(单元 + Testcontainers,需 Docker)
./gradlew packageDmg   # macOS 安装包(build/compose/binaries/main/dmg/)
./gradlew packageMsi / packageDeb   # Windows / Linux(需在对应平台构建)
```
...
- 业务逻辑(方言/扫描/仓储/服务)从 `../src/main/java` 逐行平移,类名/方法名保持一致,SQL 一字未改
