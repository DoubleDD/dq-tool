<template>
  <div class="page-card card-fill">
    ...
    <!-- 步骤 1:选择基准表(四栏级联:数据源 → 数据库 → 模式 → 表,模式栏按数据库类型动态显示),
         ...
         step-scroll:开启反查字典或窗口较矮时内容会超过卡片固定高度,步骤区内滚动,
         避免溢出内容盖住底部「上一步/下一步」按钮 -->
    <div v-show="step === 0" class="step-body step-fill step-scroll">
...
/* 第 1 步:卡片高度固定,开启反查字典/窗口较矮时固定高子块(级联 240 + 字段表 180 +
   字典选择器 200)会超出步骤区,让内容在步骤区内滚动,而不是溢出盖住底部按钮 */
.step-scroll {
  overflow-y: auto;
}
