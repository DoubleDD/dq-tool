...
- [AI 功能与表标记](docs/wiki/AI功能与表标记.md) — AI 表说明、AI 自动打标(AutoTagService,已有标记表 SKIP/APPEND/OVERWRITE 三模式;`_copy/_bak/_backup/_tmp` 备份表直接跳过并由扫描联动打「备份表」系统标记)、全局表标记与标记统计页
...
- [对象管理(数据目录)](docs/wiki/对象管理.md) — 按数据源隔离的目录树:左侧目录树展示到关系表叶子(点目录=右侧 列表/关系图/图谱 三页签,点挂载表=右侧其关系表清单,关系表不可点),树支持关键字搜索与展开/收起所有;目录挂载表(四元组,可带包含/关联关系类型)+ 挂载表登记关系表,注释从 meta_table 补,删目录 tx 级联,挂载/关系幂等;变更所属两个等价入口:树内拖动(目录/表→目录,关系表行→树上表节点)与「移动」按钮弹窗(POST .../move,防环/防重名/防重复登记);目录同级拖动排序(默认按创建时间),挂载表按挂载时间/关系表按登记时间排序,左树·列表·关系图同层节点共用这份顺序
- [Word 报告导出](docs/wiki/Word报告导出.md) — poi-tl 数据调研报告:封面+四章正文、DONE 快照口径、409 前置校验、模板改造脚本
- [数据源](docs/wiki/数据源.md) — 连接信息加密存储、SSH 隧道、库过滤白名单、JSON/Navicat/DataGrip 导入导出、连接状态标记(网络不可达分类)与断网降级本地 H2 缓存、元数据批量同步(刷新按钮)
- [元数据导入导出](docs/wiki/元数据导入导出.md) — 元数据缓存(结构缓存 + 标注数据)打包 JSON 跨机交接:导出给连不上业务库的人离线使用 ER/图谱/对象管理;结构缓存整粒度替换、派生数据按自然键合并、行袋格式前向兼容
...
- [数据比对](docs/wiki/数据比对.md) — 基准表 vs 多目标表先按对象匹配逻辑对齐(EXACT 编码+名称 / 先编码后名称 / 残余交大模型归一化)再逐字段比对:差异明细(SAME/DIFF/MISSING/EXTRA)与四比率质量报告、matchObjects/diffObjects 纯函数、差异 xlsx 导出、归档/重跑;对比模式二选一(行级=仅身份字段人工连线 / 列级=全量字段+大模型预生成映射人工审核,compare_mode V50);受控功能,授权码需显式含 compare
- [系统诊断](docs/wiki/系统诊断.md) — 排错中心:环境/授权/AI/数据源连通实测/失败记录/错误日志聚合,Markdown 诊断报告导出
- [SQL 控制台](docs/wiki/SQL控制台.md) — 类 DataGrip 执行 SQL:选数据源、执行选中片段、结果集上限 1000 行/单元格截断同数据预览、受影响行数、localStorage 历史(不落服务端);另含只读的「本地 H2 库」(应用自身配置库,语句级只读校验,写语句 400)
- [局域网共享](docs/wiki/局域网共享.md) — UDP 广播自动发现同网段实例(广播不可用的网络支持按 IP 手动添加,HTTP 直连探测),互拉标记/表描述/表所属系统与全部扫描记录(复用 Transfer 口径合并);实例身份存 system_settings,共享出口无鉴权仅限可信内网
- [授权码](docs/wiki/授权码.md) — 离线 Ed25519 授权码、签发工具、管理员实例授权码管理;菜单列表勾选即展示(旧码按功能段推导)、免接口鉴权标记(演示用)
...
- [导出中心实施计划](docs/plans/导出中心-实施计划.md) — **待办**:15 处导出统一登记可查,export_record 元数据 + 报告/抽样任务表 UNION 归一,登记制不落盘
- [后台任务中心实施计划](docs/plans/后台任务中心-实施计划.md) — 关系推导/数据比对后台异步化:统一跟踪器(kind 注册表)+ 头栏指示器 + 任务抽屉 + 终态通知(active 接口 + PENDING 态,已完成)
