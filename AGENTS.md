...
- [对象管理(数据目录)](docs/wiki/对象管理.md) — 按数据源隔离的目录树:左侧目录树常驻 + 右侧 列表/关系图(根=选中目录)/图谱(目录内表 ER 关系,只读)三页签联动;目录挂载表(四元组,可带包含/关联关系类型,展示在目录图连线上)+ 挂载表登记关系表,注释从 meta_table 补,删目录 tx 级联,挂载/关系幂等;目录同级支持拖动排序(默认按创建时间),挂载表按挂载时间/关系表按登记时间排序,左树·列表·关系图同层节点共用这份顺序
