...
- [数据比对](docs/wiki/数据比对.md) — 基准表 vs 多目标表先按对象匹配逻辑对齐(EXACT 编码+名称 / 先编码后名称 / 残余交大模型归一化)再逐字段比对:差异明细(SAME/DIFF/MISSING/EXTRA)与四比率质量报告、matchObjects/diffObjects 纯函数、差异 xlsx 导出、归档/重跑;受控功能,授权码需显式含 compare
