...
- **tauri 安装包自动更新**(tauri-plugin-updater,详见 tauri/AGENTS.md):windows-tauri 任务额外产出 `.nsis.zip` 更新包与 `latest.json` 清单并挂 Release,应用启动时后台预下载、弹窗确认后重启生效;签名私钥直接入库 `scripts/updater-private.key`(单行 base64 无密码,分发方多机打包需要;仓库公开,验签仅作形式约束),公钥在 tauri.conf.json;**每次发版必须递增 tauri.conf.json 的 version(semver 比较)**
