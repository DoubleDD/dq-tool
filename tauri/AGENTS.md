...
- **自定义 IPC 命令**:webview 加载本地来源(`frontendDist`),前端通过
  ...
  **ureq 是唯一 HTTP client**(阻塞式,不引 async runtime),仅为本命令引入;
  ...
  壳过旧靠就绪失败回滚兜底;开发模式(无 versions 目录)与 macOS 在命令入口拦截报错;
  前端入口在系统设置页「应用更新」卡片(仅 Tauri 形态显示)
