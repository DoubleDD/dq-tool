# AGENTS.md — desktop(Compose Desktop 桌面版)

> 本文件面向 AI 编码代理。这是 dq-tool 的 Kotlin + Compose Desktop 重写版,项目文档与代码注释统一使用中文。原 Web 版(Spring Boot + Vue)见仓库根目录的 AGENTS.md。本目录是根 Gradle 多模块构建的子模块(`:desktop`),Gradle 命令统一在仓库根目录执行。
...
- 根 Gradle wrapper 9.2.1,JDK toolchain 25;共享依赖(JDBC 驱动/POI/H2/testcontainers)版本统一在根 `gradle/libs.versions.toml`

## 项目结构

```
build.gradle.kts           全部依赖与打包配置(7 个 JDBC 驱动等共享依赖版本在根 gradle/libs.versions.toml 统一管理)
...
```bash
./gradlew :desktop:run / :desktop:test / :desktop:packageDmg   # 在仓库根目录执行
```

- Testcontainers 需 Docker;OrbStack 用户:`DOCKER_HOST=unix://$HOME/.orbstack/run/docker.sock ./gradlew :desktop:test`
