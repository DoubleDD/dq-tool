...
class OracleDialect : AbstractDialect() {
    ...
    companion object {
        ...
        internal fun listTablesSql(major: Int): String {
            val sizeExpr = if (major >= 23)
    ...
    override fun sumSizeBySchema(conn: Connection): Map<String, Long> {
        ...
        if (oracleMajor(conn) >= 23) {
            conn.createStatement().use { st ->
                st.executeQuery("SELECT SYS_CONTEXT('USERENV','SESSION_USER'), " +
                        "(SELECT SUM(bytes) FROM user_segments) FROM dual").use { rs ->
                    rs.next()
                    return mapOf(rs.getString(1) to rs.getLong(2))
                }
            }
        }
    ...
    override fun listTables(conn: Connection, schema: String): List<TableStat> {
        ...
        val sql = listTablesSql(oracleMajor(conn))
