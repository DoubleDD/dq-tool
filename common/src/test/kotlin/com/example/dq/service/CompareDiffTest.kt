...
class CompareDiffTest {
    ...
    fun `字段不一致记 DIFF 并逐字段展开`() {
        ...
        val result = CompareService.diffObjects(base, target, listOf("id", "name", "amount"), listOf("id"))
    ...
    fun `完全一致记 SAME`() {
        ...
        val result = CompareService.diffObjects(base, target, listOf("id", "name"), listOf("id"))
    ...
    fun `数值字段千分位与小数尾零归一后视为一致`() {
        ...
        val result = CompareService.diffObjects(base, target, fields, listOf("id"), numericFields = numeric)
    ...
    fun `数值字段不同值记不一致`() {
        ...
        val result = CompareService.diffObjects(base, target, listOf("id", "amount"), listOf("id"),
