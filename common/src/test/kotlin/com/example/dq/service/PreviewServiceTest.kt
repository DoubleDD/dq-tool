...
/**
 * 预览全量导出(exportTable):H2 内存库实测 WHERE 过滤、ORDER BY 排序、NULL 空单元格。
...
class PreviewServiceTest {
    ...
    /** 导出并用 POI 读回,返回 (表头行, 数据行) 的纯文本矩阵 */
    private fun exportRows(where: String?, orderBy: String?): Pair<List<String>, List<List<String>>> {
        ...
        XSSFWorkbook(ByteArrayInputStream(out.toByteArray())).use { wb ->
            ...
            val header = rowText(0)
            val rows = (1..sheet.lastRowNum).map { rowText(it) }
    ...
    fun `无条件导出全部行且保持 ORDER BY 排序`() {
        val (header, rows) = exportRows(null, "id desc")
        assertEquals(2, header.size)
        assertEquals("ID", header[0].split(" ")[0])
        assertEquals("NAME", header[1].split(" ")[0])
        assertEquals(listOf(listOf("3", "c"), listOf("2", ""), listOf("1", "a")), rows)
    }

    @Test
    fun `WHERE 过滤只导出符合条件的行且剥离误带前导关键字`() {
        val (header, rows) = exportRows("WHERE id > 1", "id")
        assertEquals(2, header.size)
