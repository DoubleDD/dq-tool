...
/** JDBC URL 主机端口解析与改写:8 种方言、Oracle SID 形态、缺省端口回落、不可解析报错 */
class JdbcUrlRewriterTest {
    ...
    fun `八种方言带端口 URL 解析`() {
        ...
        assertEquals("10.0.0.8" to 5867, JdbcUrlRewriter.extractHostPort("jdbc:highgo://10.0.0.8:5867/app"))
    }
    ...
    fun `缺省端口按库类型回落默认端口`() {
        ...
        assertEquals("h" to 5866, JdbcUrlRewriter.extractHostPort("jdbc:highgo://h/app"))
    }
