...
src/main/kotlin/com/example/dq/
  ...
  repository/              Jdbc.kt 薄封装 + MetaWriteQueue.kt(元数据缓存统一写队列:全局单线程 FIFO,所有 meta_*/schema_stat 写操作经它串行执行,对 H2 元数据表禁止并发写;submit 阻塞等结果=读路径,submitAsync 火忘提交=扫描路径,worker 不等写库)+ 16 仓储(CompareImportRepository:比对导入批次 compare_import 读写,BUILDING/DS_REVIEW/DONE/FAILED 状态机,重启残留 BUILDING 置 FAILED;CompareRepository 任务查询统一左联 compare_import 取来源文件名;TableRelationRepository:ER 表间关系双向存储——table_relation 为最终关系(方向化唯一侧 one/重复侧 many,三态 CANDIDATE/CONFIRMED/REJECTED,新增 reviewed/reviewed_at 标记人工已审核;唯一键幂等 insertIfAbsent;reviewBatch 批量审核+逐条备注),table_relation_original 为大模型推导产出快照(人工审核不改变,导出时与最终关系做差得「关系变化」);RelationInferJobRepository:推导任务进度/终态,启动 failRunningOnStartup 清残留 RUNNING;TableSystemRepository:表所属系统四元组存储,一表一系统,upsert 幂等 + 批量清除;MetaCacheRepository:库/schema 清单(meta_database)+ 表/字段/索引结构本地缓存 + 整库字段清单缓存(meta_schema_column,SQL 控制台智能提示用,支持 schema 级整库覆盖与按表分批覆盖)+ 单表建表 DDL(meta_ddl)与 schema 字段总数(meta_column_count)缓存(断网降级用,行存在即就绪、由结构覆盖刷新按粒度失效),懒加载 + 手动/扫描刷新,meta_cache_flag 区分「未缓存」与「已缓存但为空」;写操作统一走 MetaWriteQueue 全局串行(禁止并发写 H2),replaceTables 整粒度覆盖表清单时清理已删表的子级缓存;**扫描只写不读该缓存**(结构真源永远是原始库,每次扫描/续扫都整粒度覆盖);AiUsageRepository:AI 调用用量流水——独立 H2 库 dqaiusage,含请求/响应内容,记录时快照扫描标签不跨库 join,启动时一次性搬迁主库老数据 migrateLegacyIfEmpty,按日/按场景/按扫描聚合 + 最近明细分页;CompareAiTraceRepository:比对 AI 判定留痕(与用量同库,insert/listByJob/deleteByJob,所有写操作内部 try-catch 只记 warn 绝不上抛——留痕失败不炸比对主流程));ErrorRepository:错误中心单表读写(fingerprint 唯一 upsert 聚合计数、条件分页/统计/批量状态标记/按天数或上限清理))+ SchemaInit(Flyway 迁移封装,location 参数区分主库/AI 用量库脚本目录)
