...
src/main/kotlin/com/example/dq/
  ...
  repository/              Jdbc.kt 薄封装 + 12 仓储(MetaCacheRepository:库/表/字段/索引结构本地缓存 + 整库字段清单缓存(meta_schema_column,SQL 控制台智能提示用,支持 schema 级整库覆盖与按表分批覆盖),懒加载 + 手动/扫描刷新,meta_cache_flag 区分「未缓存」与「已缓存但为空」;AiUsageRepository:AI 调用用量流水——独立 H2 库 dqaiusage,含请求/响应内容,记录时快照扫描标签不跨库 join,启动时一次性搬迁主库老数据 migrateLegacyIfEmpty,按日/按场景/按扫描聚合 + 最近明细分页)+ SchemaInit(Flyway 迁移封装,location 参数区分主库/AI 用量库脚本目录)
