...
src/main/kotlin/com/example/dq/
  ...
  repository/              Jdbc.kt 薄封装 + 15 仓储(TableRelationRepository:ER 表间关系双向存储——table_relation 为最终关系(方向化唯一侧 one/重复侧 many,三态 CANDIDATE/CONFIRMED/REJECTED,新增 reviewed/reviewed_at 标记人工已审核;唯一键幂等 insertIfAbsent;reviewBatch 批量审核+逐条备注),table_relation_original 为大模型推导产出快照(人工审核不改变,导出时与最终关系做差得「关系变化」);RelationInferJobRepository:推导任务进度/终态,启动 failRunningOnStartup 清残留 RUNNING;TableSystemRepository:表所属系统四元组存储,一表一系统,upsert 幂等 + 批量清除;MetaCacheRepository:库/schema 清单(meta_database)+ 表/字段/索引结构本地缓存 + 整库字段清单缓存(meta_schema_column,SQL 控制台智能提示用,支持 schema 级整库覆盖与按表分批覆盖)+ 单表建表 DDL(meta_ddl)与 schema 字段总数(meta_column_count)缓存(断网降级用,行存在即就绪、由结构覆盖刷新按粒度失效),懒加载 + 手动/扫描刷新,meta_cache_flag 区分「未缓存」与「已缓存但为空」;AiUsageRepository:AI 调用用量流水——独立 H2 库 dqaiusage,含请求/响应内容,记录时快照扫描标签不跨库 join,启动时一次性搬迁主库老数据 migrateLegacyIfEmpty,按日/按场景/按扫描聚合 + 最近明细分页)+ SchemaInit(Flyway 迁移封装,location 参数区分主库/AI 用量库脚本目录)
