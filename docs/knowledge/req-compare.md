...
- 双侧任一残余超过 `LLM_RESIDUE_LIMIT`(20000,失控保险丝)→ 不调用,说明「残余对象过多…未做归一化补配」;
- 三段式(每条残余要么被配对、要么经语义判读才能定性未匹配):
  1. 本地归一化精确补配(零成本):编码 trim+lowercase 相等 → CODE;名称归一化(trim/小写/去空白/
     全角转半角/剥括号补充说明)相等 → NAME(`appendNormalizedPairs`);
  2. 相似度召回(零成本):目标侧「名称 bigram + 编码字母/数字段」倒排索引,每条基准按
     「2×名称 Jaccard + 包含关系分 + 编码共享加分」取 top 20 候选;候选池超 2000 按命中次数截断
     (`recallCandidates`);与所有目标零字符交集的基准召回不到候选;
  3. 模型裁决:按条目预算 600/请求(每条基准 1 + 其候选数)贪心装批(`packCandidateBatches`),
     每条基准只带自己的候选列表(`buildCandidateMatchPrompt`),要求输出 `[{"b":基准序号,"t":目标全局序号}]`;
     **只采纳落在该基准候选集内的配对**(`filterPairsByCandidates`,防跨候选乱配);
- 说明如实披露召回边界:无字符交集的基准、未被任何基准召回的目标计数写进 note(极端别名可能漏配,需人工核对);
...
- `CompareMatchTest`(纯函数 + H2 上的 AiConfigService):三种匹配逻辑的配对口径(编码忽略大小写、
  ...
  实例侧补配(无残余不调用、未配置大模型给说明、归一化精确补配、召回候选/零交集披露、
  候选集外配对丢弃、失败只跳批、超 20000 条跳过、条目预算分批)。
...
- 大模型补配的耗时不受「单侧 50 万行」保护:请求数 ≈ 残余数 ÷ 每批基准数(每条约 21 条目预算),
  残余越大批次数越多 × 单次 120s 超时可能拖很久,
  期间进度条停在「比对 XX…」不动属预期;
...
flowchart TD
    ...
    subgraph BG["后台线程 compare-N"]
        ...
        K -->|是| K1["aiMatchResidues 归一化补配双侧残余<br/>归一化精确配 → 相似度召回 → 模型裁决<br/>未配置/超 20000/批失败均降级记 note"]
