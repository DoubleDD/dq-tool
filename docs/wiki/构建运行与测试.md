...
启动时序与就绪探针(2026-08 免安装版白屏优化):WebServer 构造只做内核对象图 + 路由(纯内存装配),`start` 绑定端口后即开窗、静态页面秒出;共享内核重活(H2 建表/迁移 + 中断/报告任务恢复,ServiceEnv.initDatabase)随后完成。期间业务接口由就绪闸门统一返回 503 + Retry-After,`/api/health` 返回 503 starting;前端 `main.js` 挂载应用前轮询 `/api/health`(500ms 间隔,60s 超时兜底),轮询到 200 才 mount 并加载数据,页面停留在 index.html 的「正在连接服务」占位,避免白屏与接口报错刷屏。
