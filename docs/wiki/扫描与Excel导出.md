...
- 结构元数据本地缓存(库/表/字段/索引):库列表统计(schema_stat)、表清单/字段/索引(meta_table/meta_column/meta_index)首次访问从业务库拉取落 H2,之后浏览读缓存不连业务库;库列表/表列表/字段明细页各有「刷新」按钮(接口带 `?refresh=true`)强制从数据源拉最新结构并覆盖缓存;发起扫描时顺带刷新表/字段/索引缓存(任务引导阶段刷表清单——全量清单整粒度覆盖 `replaceTables`,只扫若干张表时按表名局部合并 `mergeTables`、不把全量缓存冲成部分清单;`planTable` 刷字段+索引;失败不影响扫描);覆盖刷新为「先 DELETE 后 INSERT」;所有元数据缓存写入统一走 `MetaWriteQueue` 全局单线程队列串行化(扫描多 worker 并发刷不同粒度也只有一个 H2 写者),避免互相等锁与唯一键冲突(23505);`replaceTables` 整粒度覆盖表清单还会清掉已删除表的子级缓存(字段/索引/整库字段清单/DDL/就绪标记)。**扫描只写不读缓存**:规划/续扫的结构一律现查原始库,拿到最新字段+索引后整粒度覆盖缓存(空字段表也覆盖为「空 + 就绪」并置无字段标记),绝不从 H2 读结构(详见 [数据源](数据源.md))
...
- **建任务是异步的**:`POST /api/scans` → `ScanService.createScan` 只做本地校验(数据源存在)与落任务行(PENDING、`total_tables=0`),随即返回 jobId;拉表清单(必连业务库)、结构缓存同步、`scan_table` 落库全部交给后台 `bootstrapJob`(提交到 `ScanExecutor`),详情页轮询自会看到表清单陆续出现。**动机**:这几步原本跑在 HTTP 请求线程上,远程/大 schema(如 Oracle APEX_* 上千张表)的字典查询、尤其半死库的建连会让接口卡几十秒(实测未设超时时 Oracle 建连耗到 84s 才报 ORA-17800,期间前端 30s axios 超时已报错、后端还在跑)。**代价**:「选中的表不存在」「没有不超过大小上限的表」这类校验由同步抛错(HTTP 4xx)改为任务落 FAILED + `error` 文案,详情页「错误信息」展示;引导完成后回填 `total_tables` 与 `db_version`
- 只扫少量表时**按表名过滤字典查询**:`DbDialect.listTables(conn, schema, tableNames)`(各方言把 `AND <表名> IN (...)` 拼进原字典 SQL),上限 500 张(`ScanService.MAX_FILTERED_TABLES`;Oracle IN 列表 1000 上限留余量),超过则退回整库拉取后内存过滤。Oracle/DM 的表清单 SQL 体积要关联段视图,整库拉取代价高——「只扫一张表」不该付整库的字典成本;过滤路径不是全量清单,故只 `mergeTables` 局部合并缓存
- 业务库连接设了**建连/读取超时**(`dq.scan.db-connect-timeout-seconds` 默认 15s、`dq.scan.db-read-timeout-seconds` 默认 1800s,<=0 不限制):由 `DbDialect.connectionTimeoutProperties` 按驱动给出属性名(MySQL connectTimeout/socketTimeout、PG 系 connectTimeout/socketTimeout 秒、SQL Server loginTimeout/socketTimeout、Oracle `oracle.net.CONNECT_TIMEOUT`/`oracle.jdbc.ReadTimeout`),`DataSourceService` 在建池与一次性试连(连接测试/维护库探测)两处统一透传;达梦未覆盖(驱动属性名未验证),走驱动默认。没有这层约束时半死库会把请求线程拖到驱动默认(分钟级)
- 扫描可选「生成表描述」(`gen_doc` 随 scan_job 持久化,默认开):每张表 DONE 后由 `ScanDocService` 独立守护线程池(2 worker)异步调 `TableDocService.generate` 生成 AI 表说明落 table_doc;已有非空描述/未配置大模型/任务取消或失败均跳过,同 job 首次 LLM 失败后熔断剩余表;前端扫描对话框复选默认勾选,提交前统一校验 AI 可用性(配置完整性+连通实测,不可用弹确认框提示将跳过,口径见 [AI功能与表标记](AI功能与表标记.md))
...
- 业务库执行的 SQL 全部打日志(独立 logger `com.example.dq.sql`,默认 INFO):`DataSourceService` 连接出口统一 JDK 代理包装(`SqlLogConnection`),拦截 Statement/PreparedStatement 的 execute 类调用打印完整 SQL 与绑定参数(SQL 文本超 8000 字符、单参数渲染超 1000 字符、ByteArray 超 64 字节均截断并附截断标记,防超大日志行);排查慢 SQL/深分页等场景用,日志文件按天滚动可回溯。本地 H2(repository 包)不走该出口,不打日志;不需要时把 logback 中 `com.example.dq.sql` 调为 WARN/OFF。代理反射调用会拆包 `InvocationTargetException` 原样透出底层 `SQLException`(否则被包成 `UndeclaredThrowableException`,方言层 catch(SQLException) 的降级逻辑会失效)
...
- 规划阶段(`planTable`)发现表不存在或没有字段(`listColumns` 为空)不算失败:直接按空表置 DONE、结果全 0(`ChunkRunner.completeEmptyTable`),联动「空表」「备份表」系统标记但不触发 AI 后续(无字段无可分析);续扫(`resume`)同样按此跳过——旧行为是抛错让整个任务 FAILED,现已改为只跳过该表(FAILED 表翻转为 DONE 时不重复计入完成数)
...
- 采样行数支持任务级覆盖:扫描弹窗「采样行数」(留空=全局默认 `dq.scan.sample-rows`/系统设置,非法值 <1 钳到 1 与系统设置保存口径一致),落库 `scan_job.sample_rows`(V53,NULL=用全局默认,存量兼容);规划采样表时把 `job.sample_rows ?: 全局默认` 快照进 `scan_table.sample_rows`,执行侧 ChunkRunner 本就有「表级快照优先」取值,断点续扫重规划缺失表时读 job 行,口径不变
...
- Oracle 表体积 SQL 由逐表相关子查询改为**分组聚合 + LEFT JOIN**:`all_segments` 按 `owner+segment_name` 聚合、索引段经 `all_indexes` 关联段视图后按 `owner+table_name` 聚合,再与 `all_tables` 左连相加(`OracleDialect.listTablesSql`)。原写法 `(SELECT SUM(...) WHERE segment_name=t.table_name OR segment_name IN (子查询))` 对 all_tables 每一行都要跑一遍段视图,组合复杂度约 O(表数 × 段数);聚合版每张段视图只扫一遍。语义不变:表段与索引段皆不可见时体积仍为 NULL(未知)
...
- 达梦表/库体积统计同样依赖段视图,降级机制与 Oracle 同一套(`DmDialect.sizeSourceCache` 按账号缓存、1 小时重探,降级记 warn 日志),降级链为五环:ALL_SEGMENTS → DBA_SEGMENTS → TABLE_USED_SPACE 系统函数(体积 = 已用页 × `PAGE()`,不依赖段视图,口径为已用空间而非分配字节,仅作兜底)→ USER_SEGMENTS(仅当前用户)→ 不统计。函数环排在 USER_SEGMENTS 之前:受限账号下 USER_SEGMENTS 能查成功但只覆盖本账号,会提前截断链路让其他 schema 体积永远空白(实测:WI_USER_ONLY 账号 ALL_SEGMENTS 报「无效的表或视图名」SQLState 22000、DBA_SEGMENTS 报无查询权限 28000);部分实例或受限账号下旧版是静默吞异常导致体积列空白且无日志;达梦对视图不存在与无权限的报错口径随实例配置不一,降级时不细分错误码一律下移一环。表清单 SQL 也一并从逐表相关子查询改为分组聚合 + LEFT JOIN(同 Oracle 口径)
...
把扫描记录(任务 + 事件时间线 + 表级/分段/字段明细)导出为 JSON 文件,在另一台机器的部署一键导入,扫描记录列表即可看到导入的历史记录。导出还随任务携带每张表的表标记(USER 标记 + 空表/备份表系统标记关系)与表描述(标注数据),导入成功一个任务后随即合并进本机全局标记/描述,避免换机后重新打标与重新生成描述、也避免导入的历史记录丢失系统标记。实现:`ScanTransferService`(common)+ `ScanTransferController`(server),前端入口在扫描记录页(`Scans.vue`)工具栏「导出记录/导入记录」。
