...
- 只扫少量表时**按表名过滤字典查询**:`DbDialect.listTables(conn, schema, tableNames)`(各方言把 `AND <表名> IN (...)` 拼进原字典 SQL),上限 500 张(`ScanService.MAX_FILTERED_TABLES`;Oracle IN 列表 1000 上限留余量),超过则退回整库拉取后内存过滤。Oracle/DM 的表清单 SQL 每行都要做体积子查询,整库拉取代价极高——「只扫一张表」不该付整库的字典成本;过滤路径不是全量清单,故只 `mergeTables` 局部合并缓存
