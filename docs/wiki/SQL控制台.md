...
- **布局**:页面纵向 flex 铺满主区域,编辑器区(编辑器 + 右侧表清单)占满剩余高度(最低 220px,超出各自内部滚动),结果区按需占位(上限约半页,超高内部滚动)。
- **右侧表清单**:选定数据源+库后,编辑器右侧定宽面板(250px)列出该库的表(两行式:英文表名等宽字体 + 表注释,悬停 title 显示全称),头部显示「表清单 · N」;数据与智能提示**同一份拉取结果**,随数据源/库下拉联动刷新、刷新按钮同样覆盖;多库方言(SQL Server/Kingbase)按 `schema.table` 限定名列出;未选库时不显示面板。
