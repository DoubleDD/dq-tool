...
| A10 | 导出中心「打开目录/打开文件」在 Windows 打开了「此电脑」(像 C 盘)而非数据目录 | 中 | **已修复**(2026-09-22) | 全形态(Windows) |
...
### A10. Windows「打开目录」静默失败开成「此电脑」(已修复,2026-09-22)

**现象**:Tauri 绿色版(Windows)导出中心点「打开目录」,资源管理器打开的是「快速访问/此电脑」
(观感像 C 盘根目录),而不是数据目录下的 `exports/`。

**根因与判据**:不是数据目录传错——后端 `resolveInDataDir` 对不存在的路径会直接报错走前端 toast,
窗口能弹出说明 Java 侧校验通过、传给 explorer 的是正确绝对路径。问题在最后一跳
`SystemOpen.reveal` 的 `explorer /select,<路径>`:explorer 的开关解析按 **ASCII 逗号切分**(引号救不回)、
**超 MAX_PATH(260)** 打不开、空格路径不加内层引号也可能被截断;失败时**静默打开默认位置**(此电脑)。
绿色版常被解压到很深的自定义目录,路径长度/特殊字符踩中率远高于安装版。
判据:日志里 `SystemOpen.reveal` 每次打印「已打开文件目录： <完整路径>」,路径正确但窗口开错即坐实本问题;
若路径本身指向 `~/.dq-tool`,则是 `PORTABLE.txt` 丢失导致绿色版判定失效(见 A8/D3)。

**处置**(`common/src/main/kotlin/com/example/dq/util/SystemOpen.kt` `revealOnWindows`):
路径按微软文档加内层引号(`/select,"<path>"`);路径含 ASCII 逗号或超 250 字符时退化为
`explorer <所在目录>`(打开目录、不选中文件),不再触发静默兜底。

**回归**:Windows 真机把绿色版解压到带空格/较深的目录,导出一次后点「打开目录」,
确认打开的是 `<exe>\data\exports\` 且选中文件。
