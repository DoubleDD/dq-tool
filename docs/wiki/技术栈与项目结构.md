...
server/                      Java Web 版薄壳(Javalin):
  ...
  src/main/java/com/example/dq/
    ...
    web/                     WebServer(装配内核 ServiceEnv + Javalin 路由注册 + 授权 beforeMatched 前置校验 + 统一异常映射为 {message}(401/400/409/502/500,前端 axios 拦截器直接弹 message)+ 静态资源与 SPA 回退)、LogStreamAppender(编程挂载到 root logger 的实时日志 Appender,环形缓冲最近 500 条 + 推送给 SSE 订阅者)、LogEntry(日志条目标记录)、Validators(@Valid 替代的校验工具)、ValidationException(继承 IllegalArgumentException,走 400 映射)
